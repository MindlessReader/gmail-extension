...
setTimeout(function() {
    ...
    var suspiciousWordsExt = ["password", "malware", "vulnerability", "critical vulnerability",
        ...
        "receiving payment", "control computer", "full access", "darknet", "darkweb", 
        "post social network", "publish everything", "case-sensitive", "case sensitive", "case-sensetive", 
        "case sensetive", "private data"
    ]; //Extortion
    ...
    suspiciousWordsRom.forEach(element => {
        if (searchString(body, element) != null && searchString(body, element) != undefined) {
            spamScoreRom = spamScoreRom * (searchString(body, element).length * 2);
        }
    });

    suspiciousWordsExt.forEach(element => {
        if (searchString(body, element) != null && searchString(body, element) != undefined) {
            spamScoreExt = spamScoreExt * (searchString(body, element).length * 2);
        }
    });
